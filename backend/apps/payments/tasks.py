...
def export_financial_report(academy_id, user_email):
    ...
    payments = Payment.objects.filter(academy_id=academy_id).select_related('invoice__player', 'recorded_by')
    
    csv_buffer = io.StringIO()
    writer = csv.writer(csv_buffer)
    writer.writerow(['Payment ID', 'Date', 'Amount', 'Method', 'Player', 'Invoice ID', 'Recorded By'])
